...
function DiscordLib:Window(text,cls)
...
local win = DiscordLib:Window("PWANS", _G.Toggle)

local serv = win:Server("BloxFruit v0.8", "")

local farm = serv:Channel("AutoFarm")

local esp1 = serv:Channel("ESP")

local raid = serv:Channel("Raid")

local misc = serv:Channel("Misc")

local sett = serv:Channel("Settings")

local oblv = serv:Channel("Observation Level")
...
esp1:Toggle("Fruit ESP",false, function(Value)
    DevilESP = Value
...
esp1:Toggle("Player ESP",false, function(Value)
    PlayerESP = Value
...
raid:Toggle("Kill All",false, function(vu)
    Killaura = vu
end)
...
misc:Button("Bring Fruit", function()
    for i,v in pairs(game:GetService("Workspace"):GetChildren()) do
                    if string.find(v.Name, "Fruit") then
                    if v:IsA("Tool") then
...
misc:Button("Buy Random Fruit", function()
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Cousin","Buy")
    wait(1)
    DiscordLib:Notification("Buy Random Fruit", "Fruit Bought!", "Exit")
end)

misc:Seperator()

misc:Button("Buy Buso Color", function()
    local args = {
		[1] = "ColorsDealer",
		[2] = "2"
		}
...
farm:Toggle("AutoFarm",false, function(vu)
    _G.AutoFarm = vu
	if _G.AutoFarm and SelectToolWeapon == "" then
		DiscordLib:Notification("AutoFarm","Select Weapon First!","Okay")
	else
		Auto_Farm = vu
		SelectMonster = ""
		if vu == false then
...
local SelectWeapon = farm:Dropdown("Select Weapon",Wapon,function(Value)
...
sett:Toggle("Press Home Button Stop Tween",false, function(vu)
...
spawn(function()
    game:GetService("RunService").Heartbeat:Connect(function()
        if HolyTorch or Auto_Farm then
...
local ObservationLevel = oblv:Label("")
